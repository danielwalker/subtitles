...
- (id)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        // We need to return our containerView as the view for zooming, and we also need to receive
...
- (void)setMinimumResolution:(int)res {
	minimumResolution = MIN(res, 0); 
} 

- (void)setMaximumResolution:(int)res { 
	maximumResolution = MAX(res, 0); 
}
...
- (void)reloadDataWithNewContentSize:(CGSize)size {
    ...
    // we also need to change the frame of the containerView so its size matches the contentSize
    [containerView setFrame:CGRectMake(0, 0, size.width, size.height)];
    [self reloadData];
}
...
- (void)layoutSubviews {
    ...
    for (UIView *tile in [containerView subviews]) {
        ...
        CGRect scaledTileFrame = [containerView convertRect:[tile frame] toView:self];
    ...
    int maxRow = floorf([containerView frame].size.height / scaledTileHeight); // this is the maximum possible row
    int maxCol = floorf([containerView frame].size.width  / scaledTileWidth);  // and the maximum possible column
    ...
    for (int row = firstNeededRow; row <= lastNeededRow; row++) {
        for (int col = firstNeededCol; col <= lastNeededCol; col++) {
            ...
            if (tileIsMissing) {
                ...
                // annotateTile draws lines and tile numbers on the tiles for illustration purposes. 
				if(ANNOTATE_TILES){
...
- (void)updateResolution {
    ...
    if (delta != 0) {
        ...
        [containerView setFrame:CGRectMake(0, 0, containerSize.width, containerSize.height)];    
...
- (void)scrollViewDidEndScrollingAnimation:(UIScrollView *)scrollView {
	[dataSource tiledViewDidEndScrollingAnimation];
}

- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
	[dataSource tiledViewDidScroll];
}
...
- (void)annotateTile:(UIView *)tile {
    ...
    if (!label) {  
        ...
        [label setText:[NSString stringWithFormat:@" T%d", totalTiles]];
        [tile addSubview:label];
        [label release];
        [[tile layer] setBorderWidth:1];
